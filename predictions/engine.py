...
def engine_a(data):
    ...
    ai = call_ai(
        'You are a football prediction AI. Return ONLY valid JSON, no other text.',
        f"Match: {hn} vs {an}\n"
        f"Home: form={hform:.0%} goals={hgs}/{hgc} injuries={hinj} position={hpos}\n"
        f"Away: form={aform:.0%} goals={ags}/{agc} injuries={ainj} position={apos}\n"
        f"H2H last 10: {h2hh}W {h2hd}D {h2ha}A\n"
        f"V1 engine says: Home={v1h:.1%} Draw={v1d:.1%} Away={v1a:.1%}\n"
        f'Respond with this exact JSON: {{"homeWin":0.5,"draw":0.25,"awayWin":0.25,"insight":"Your 2 sentence analysis here.","key_factor":"Main factor","predicted_score":"2-1"}}'
    )
    ...
    return {
        ...
        'insight': ai.get('insight', f'V1 analysis: {hn} vs {an}. Based on form, goals and H2H data.') if ai else f'V1 result: {verdict} predicted to win.'
    }
...
def engine_b(data):
    ...
    scores = {
        'pass':clamp(pass_acc,0,100),'tackle':clamp(tackles,0,100),
        'aerial':clamp(aerials,0,100),'dist':clamp((distance/13)*100,0,100),
        'goals':clamp((goals/games/0.7)*100,0,100),'assist':clamp((assists/games/0.4)*100,0,100),
        'shot':clamp(shots_ot,0,100),'drib':clamp(dribbles,0,100)
    }
    ...
    ai = call_ai(
        'You are a football player rating AI. Return ONLY valid JSON, no other text.',
        f'Player: {name}, Position: {pos}, Goals: {goals}, Assists: {assists}, Games: {games}\n'
        f'Pass accuracy: {pass_acc}%, Shots on target: {shots_ot}%, V1 rating: {rating:.1f}, Injury: {injury}\n'
        f'Respond with this exact JSON: {{"adjusted_rating":80,"tier":"Elite","insight":"Your 2 sentence analysis here."}}'
    )
...
def engine_d(data):
    ...
    hn = home.get('name','Home'); an = away.get('name','Away')
    ai = call_ai(
        'You are a football simulation AI. Return ONLY valid JSON, no other text.',
        f'{hn} vs {an}: {n} simulations run.\n'
        f'Results: Home {hp}% | Draw {dp}% | Away {ap}% | Likely score: {likely}\n'
        f'Respond with this exact JSON: {{"insight":"Your 3 sentence match analysis here.","key_battle":"Main area that decides the match"}}'
    )
    return {
        ...
        'insight': ai.get('insight',f'V1 simulation: {n} runs. Most likely score: {likely}.') if ai else f'V1: {n} simulations complete. Likely: {likely}'
    }
...
def natural_language(question):
    ...
    ai = call_ai(
        'You are a football prediction expert. Return ONLY valid JSON, no other text.',
        f'Football question: "{question}"\n'
        f'Respond with this exact JSON: {{"answer":"Your detailed 3-4 sentence answer here.","prediction":"Home Win","confidence":75,"key_factors":["Factor 1","Factor 2","Factor 3"]}}',
        max_tokens=500
    )
    return ai or {'answer':'AI analysis unavailable. Please use Engine A for predictions.','prediction':'Unknown','confidence':0,'key_factors':[]}
